...
public class MoveStudentMessage implements ClientMessage {
    ...
    private final Optional<Integer> island_position_optional;

    public MoveStudentMessage(Color color, Integer island_position) {
        this.color = color;
        this.island_position_optional = Optional.ofNullable(island_position);
    }
    ...
    public ServerResponse handle(NetworkManager network_manager, Player player) {
        ...
        // Invalid state. It ust be (current_state=MOVE_STUDENT, action_completed=False)
        ...
        if(island_position_optional.isPresent()){
            int island_position = island_position_optional.get();
            ...
            Island island = game.getIslands().get(island_position_optional.get());
